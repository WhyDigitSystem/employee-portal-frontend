    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (
          ...
          response.data.paramObjectsMap.organizationVO &&
          response.data.paramObjectsMap.organizationVO.branchVO
        ) {
          // Extract the branch name from the response
          const branchName =
            response.data.paramObjectsMap.organizationVO.branchVO[0].branchName;
        ...
        console.log(
          "OrganizationName:",
          response.data.paramObjectsMap.organizationVO.name
        );
        console.log(
          "OrganizationDetail:",
          response.data.paramObjectsMap.organizationVO
      ...
      <ul className="menu  pt-2 w-80 bg-base-100 text-base-content">
        ...
        <li className="mb-2 font-semibold text-xl">
          <Link to={"/app/welcome"}>
            ...
            {/* <span>{orgName}</span> &nbsp;<span style={{ fontSize: "12px" }}>{branchName}</span> */}
          </Link>
          <div>
            {orgName} &nbsp;
            <span style={{ fontSize: "12px" }}>{branchName}</span>
          </div>
