...
function PayslipPdf({ newPayslipRequest, paySlipDetails, headerDetails }) {
  ...
  const downloadPDF = () => {
    ...
    const date = new Date();
    const monthName = date.toLocaleString("default", { month: "long" }); // e.g., "February"
    const year = date.getFullYear(); // e.g., 2025

    // Construct the filename dynamically
    const fileName = `${empName}_${empCode}-${monthName}_${year}.pdf`;
          ...
          <div id="content-to-pdf" style={(styles.container, styles.body)}>
            ...
            <table style={styles.table}>
              <tbody>
                ...
                <tr>
                  <td style={styles.thTd}>Total Working Days:</td>
                  <td style={styles.thTd}>{paySlip.totalDays}</td>
            ...
            <table style={styles.table}>
              <thead>
                <tr>
                  ...
                  <th style={{ ...styles.thTd, textAlign: "center" }}>
                    Actual
                  </th>
              ...
              <tbody>
                {Array.from({
                  length: Math.max(
                    paySlip.salaryDetailsEarningsVO.length,
                    paySlip.salaryDetailsDeductionVO.length
                  ),
                }).map((_, index) => (
                  <tr key={index}>
                    <td style={styles.thTd}>
                      {paySlip.salaryDetailsEarningsVO[index]?.heading || ""}
                    </td>
                    <td style={{ ...styles.thTd, textAlign: "right" }}>
                      {paySlip.salaryDetailsEarningsVO[index]?.amount || ""}
                    </td>
                    <td style={styles.thTd}>
                      {paySlip.salaryDetailsDeductionVO[index]?.heading || ""}
                    </td>
                    <td style={{ ...styles.thTd, textAlign: "right" }}>
                      {paySlip.salaryDetailsDeductionVO[
                        index
              ...
              <tfoot>
                <tr>
                  <td style={styles.thTd}>Total Earnings: INR</td>
                  <td style={{ ...styles.thTd, textAlign: "right" }}>
            ...
            <p style={styles.netPay}>Net Payable: {paySlip.netPay}</p>
